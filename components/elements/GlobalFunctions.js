const GlobalFunctions = {
  ...
  handleCountProductsOfEachFilter: (arr, prop) => {
    ...
    for (const element of arr) {
      if (element && element["attributes"] && element["attributes"][prop]) {
        if (
          res.find(
            (e) => e.item["attributes"][prop] == element["attributes"][prop]
          )
        ) {
          res.find(
            (e) => e.item["attributes"][prop] == element["attributes"][prop]
      ...
      res.push({
        ...
        count: arr.filter(
          (element) =>
            ...
            element["attributes"][prop] > prices[i] &&
            element["attributes"][prop] < prices[i + 1]
        ).length,
      });
    }
  ...
  handleGetSumOfProductsOfCategory: (categorie) => {
    let sum = categorie["attributes"]["typeprods"]["data"].reduce(
      (sum, curValue) => {
  ...
  handleGetSumOfProductsOfUnivers: (univers) => {
    let sum = univers["attributes"]["categories"]["data"].reduce(
      (sum, curValue) => {
        return (sum +=
          GlobalFunctions["handleGetSumOfProductsOfCategory"](curValue));
      },
      0
    );
  ...
  handleGetIdsOfTypeProdssOfUnivers: (univers) => {
    return univers["attributes"]["categories"]["data"]
      ...
      .reduce((sum, curValue) => {
        return sum.concat(curValue);
      }, []);
  },

  handleSegmentArray: (array, chunkSize) => {
    const chunks = [];
