...
function Nouvellesmarques({ Exposants }) {
  ...
  /*---------------------------------------------------Hooks end---------------------------------------------------*/
  return (
    <div className="container page-content pt-50">
      ...
      {Exposants.map((exposant) => (
        <div
          key={exposant["id"]}
          className="row mb-50"
          style={{ background: "rgba(238, 238, 238, 0.3)", padding: "15px" }}
        >
          <div
            className="col-lg-3 col-md-3 col-sm-6 col-12 text-center mb-20"
            style={{
              display: "flex",
                ...
                {exposant["attributes"]["produits"]["data"].length > 0 && (
                  <div className="mb-10">
                    <span className="font-small total-product">
                      {exposant["attributes"]["produits"]["data"].length}{" "}
                      {translate("PRODUITS")}
                    </span>
                  </div>
                )}
          ...
          <div
            className="col-lg-9 col-md-9 col-sm-6 col-12"
            style={{
              display: "flex",
              justifyContent: "center",
              alignItems: "center",
...
export async function getServerSideProps(context) {
  ...
  const derniersExposantsRes = await axios.get(
    `${process.env.BASE_URL_SERVER}/api/exposants?${queryDerniersExposants}`
  );
