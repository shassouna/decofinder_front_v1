...
function Slug(props) {
  ...
  useEffect(() => {
    ...
    if (productsFiltered.length != props["Products"]) {
      ...
      router.push(
        {
          pathname: `/selection/${router.query["id"]}/${router.query["slug"]}`,
          query: { ...obj },
        },
        undefined,
        { shallow: true }
      ...
      <div className="row">
        ...
        <div className="col-lg-9">
          ...
          <div className="text-center mb-25">
            {Selection["attributes"]["expert"]["data"] && (
              <img
                src={
          ...
          <div className="mb-10">
            <Title elements={[Selection["attributes"]["titre"]]} />
          </div>
          <div
            dangerouslySetInnerHTML={{
              __html: Selection["attributes"]["description"],
            }}
            className="mb-50"
          />
...
export async function getServerSideProps(context) {
  ...
  const selectionRes = await axios.get(
    `${process.env.BASE_URL_SERVER}/api/selection-dfs/${context["params"]["id"]}?${query}`
  );

  // get localization category
  findSelection = selectionRes["data"]["data"]["attributes"]["localizations"][
    "data"
  ].find((e) => e["attributes"]["locale"] == context["locale"]);
  if (!findSelection) findSelection = selectionRes["data"]["data"];

  // Create filtres
  findSelection["attributes"]["produits"]["data"].forEach((product) => {
  ...
  return {
    props: {
      ...(await serverSideTranslations(context["locale"], ["jury", "home"])),
      Selection: findSelection,
      Products: findSelection["attributes"]["produits"]["data"],
      marques: marques,
      prices: prices,
      designers: designers,
      styles: styles,
