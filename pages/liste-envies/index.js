...
const Wishlist = () => {
    ...
    const handleDeleteFromWishlist = (product) => {
        ...
        localStorage.setItem("wishlist", JSON.stringify(wishlist.filter(element=>element["id"]!=product["id"])))
    }
                                    ...
                                    <table className="table table-wishlist">
                                        <thead>
                                            <tr className="main-heading">
                                                ...
                                                <th className="text-center" scope="col">{translate("Produit")}</th>
                                                <th className="text-center" scope="col"></th>
                                                <th className="text-center" scope="col">{translate("Exposant")}</th>
                                                <th className="text-center" scope="col">{translate("Revendeurs")}</th>
                                                <th className="text-center end" scope="col">{translate("Supprimer")}</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                        {wishlist.map(product => (
                                            <tr className="pt-30 text-center" key={product["id"]}>
                                                ...
                                                <Link 
                                                href={product["attributes"]["exposant"]["data"]?`/ss/${product["attributes"]["exposant"]["data"]["id"]}/${product["attributes"]["exposant"]["data"]["attributes"]["slug"]}`:"#"}
                                                as={product["attributes"]["exposant"]["data"]?`/ss${product["attributes"]["exposant"]["data"]["id"]}/${product["attributes"]["exposant"]["data"]["attributes"]["slug"]}`:"#"} 
                                                >  
                                                <td className="image product-thumbnail pt-40">
                                                {product["attributes"]["exposant"]["data"]&&
                                                    <img
                                                    src={product["attributes"]["exposant"]["data"]["attributes"]["logo"]["data"]?
                                                ...
                                                >   
                                                    <td className="text-center detail-info">
                                                        {product["attributes"]["exposant"]["data"]?
                                                        (
                                                            <span className="stock-status in-stock mb-0">{product["attributes"]["exposant"]["data"]["attributes"]["NOM"]}</span>
                                                        ) : (
                                                            <span className="stock-status out-stock mb-0">{translate("Pas d'exposant")}</span>
                                                        )}
                                                    </td>
                                                </Link>
                                                <td className="text-right">
                                                <button className="btn btn-sm" 
                                                onClick={(e) =>handleShowPopupRevendeurs(product)}>
                                                    {translate("Revendeurs")}                                                             
                                                </button>
                                                </td>
                                                <td className="action">
                                                    <a onClick={(e) => handleDeleteFromWishlist(product)}>
...
export async function getServerSideProps (context) {
    ...
    return {
        props: {
            ...(await serverSideTranslations(context["locale"],["liste-envies", "home"])),
        }
    }
}
